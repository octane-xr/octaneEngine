...
namespace Octane
{
    ...
    OCTANE_INLINE Quad2D CreateQuad2D()
    {
    ...
    OCTANE_INLINE Quad3D CreateQuad3D()
    {
        ...
        v1.Position = glm::vec3(0.5f, -0.5f, 0.0f); //bottom-rihgt
        v2.Position = glm::vec3(0.5f, 0.5f, 0.0f); //top-right
        v3.Position = glm::vec3(-0.5f, -0.5f, 0.0f); //top-left

        //Define normalas (in this case, all normals point in the same direction)
